...
class Child(Parent): # 定义子类
    ...
    def myMethod(self):
        print '调用子类方法 myMethod'

def run_inherit_sample():
    c = Child()          # 实例化子类
...
def main():
    ...
    emp1.displayCount()
    emp2.displayCount()
    print "Get Employee by class: %d" % Employee.empCount
...
delattr(obj, name) : 删除属性。
    '''
    hasAge = hasattr(emp1, 'age')    # 如果存在 'age' 属性返回 True。
    if hasAge:
        attr = getattr(emp1, 'age')    # 返回 'age' 属性的值
        print "age is %d" % attr
    else:
        print "has no attribute 'age'"
